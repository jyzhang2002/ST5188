...
class GRUSeq2SeqQuantile(nn.Module):
    def __init__(
        self,
        enc_input_size: int,
        dec_input_size: int,
    ...
    def forward(self, enc_inp, dec_inp):
        enc_out, h = self.encoder(enc_inp)  # h: [num_layers, B, H]
        context = h[-1] 
...
def quantile_loss_torch(preds, target, quantiles):
    B, T, Q = preds.shape
