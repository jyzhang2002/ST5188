...
def linear_layer(size: int,
    ...
    layer = nn.Linear(in_features=None, out_features=size, bias=use_bias)  # placeholder
...
class TemporalFusionTransformer(nn.Module):
    ...
    def forward(self, all_inputs: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        unknown_emb, known_emb, obs_emb, static_inputs = self._embed_inputs(all_inputs)
        ...
        hist_cat = torch.cat(hist_list, dim=-1)  # (B, enc_len, d_model, N_hist)
        ...
        if self.temporal_vsn_fut is None:
            if n_fut_vars > 0:
        ...
        hist_ctx, hist_flags, = self.temporal_vsn_hist(
            hist_cat, static_ctx_varsel)  # (B, enc_len, hidden)

        if fut_cat is not None:
            fut_ctx, fut_flags = self.temporal_vsn_fut(
                fut_cat, static_ctx_varsel)  # (B, dec_len, hidden)
        else:
            fut_ctx = torch.zeros(B, dec_len, self.hidden_layer_size, device=device)
            fut_flags = torch.zeros(B, dec_len, 0, device=device)

        h0 = static_state_h.unsqueeze(0)  # (1, B, H)
        c0 = static_state_c.unsqueeze(0)
        enc_out, (enc_h, enc_c) = self.encoder_lstm(hist_ctx, (h0, c0))  # enc_out: (B, enc_len, H)

        dec_out, _ = self.decoder_lstm(fut_ctx, (enc_h, enc_c))  # (B, dec_len, H)

        lstm_out = torch.cat([enc_out, dec_out], dim=1)  # (B, T, H)
        input_embed_full = torch.cat([hist_ctx, fut_ctx], dim=1)  # (B, T, H)
        ...
        mask = get_decoder_mask(enriched)  # (1, T, T)
        ...
        decoder_out = transformer_out[:, self.num_encoder_steps:, :]  # (B, dec_len, H)
        out = self.output_projection(decoder_out)  # (B, dec_len, out_size * num_q)

        attention_components = {
            "decoder_self_attn": self_att,   # (num_heads, B, T, T)
            "static_flags": static_flags,    # (B, num_static)
            "historical_flags": hist_flags,  # (B, enc_len, num_hist_vars)
            "future_flags": fut_flags        # (B, dec_len, num_fut_vars)
        }
    ...
    def fit(self, train_df: Optional[pd.DataFrame] = None,
        ...
        for epoch in trange(self.num_epochs):
            ...
            for b in trange(num_batches):
                ...
                preds, _ = self(xb)
                # preds: (B, dec_len, out*Q); yb: (B, dec_len, out)
